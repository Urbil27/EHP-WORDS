...
double cluster_homogeneity(float *words, struct clusterinfo *members, int wd_idx, int numclusters, int number)
{
    /****************************************************************************************
      OSATZEKO - PARA COMPLETAR
...
{
    int		i, j, numwords, k, iter, changed = 0, end_classif;
  ...
  // =====================================================
    ...
    ****************************************************************************************/
    ...
    if (cvi_zaharra != -1) { //lehenengo iterazioa ekiditu ez dugulako ezer cvi_zaharrean
        ...
        if (dif < DELTA) {
            end_classif = 1;  //Algoritmoa konbergitu du
        }
    }
    cvi_zaharra = cvi;
    
    if (end_classif == 0) {
        numclusters+=10;   //Algoritmoak ez badu konbergitu beste 10 cluster sartuko ditugu
    }
  }
